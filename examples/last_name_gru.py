...
def main():
    ...
    num_lang = len(langs)
    model = GRU(len(ALL_LETTERS), num_lang)
...
class GRU:
    def __init__(self, input_size, output_size):
        ...
        self.w_r = Tensor.rand_init(input_size + HIDDEN_SIZE, HIDDEN_SIZE)
        self.w_c = Tensor.rand_init(input_size + HIDDEN_SIZE, HIDDEN_SIZE)
        self.b_u = Tensor.rand_init(1, HIDDEN_SIZE)
        self.b_r = Tensor.rand_init(1, HIDDEN_SIZE)
        ...
        self.ones = Tensor(np.ones((1, HIDDEN_SIZE), dtype=np.float32))

    def params(self):
        return [self.w_u, self.w_r, self.w_c,
                self.b_u, self.b_r, self.b_c,
                self.w_pred, self.b_pred]

    def forward(self, input, hidden):
        combined = hidden.cat(input)
        gate_update = combined.matmul(self.w_u).add(self.b_u).sigmoid()
        gate_relevant = combined.matmul(self.w_r).add(self.b_r).sigmoid()
        c_tilde = gate_relevant.mul(hidden).cat(input).matmul(self.w_c).add(self.b_c).tanh()
        c = gate_update.mul(c_tilde).add(self.ones.sub(gate_update).mul(hidden))
        pred = c.matmul(self.w_pred).add(self.b_pred).softmax(dim=1)
        return pred, c
